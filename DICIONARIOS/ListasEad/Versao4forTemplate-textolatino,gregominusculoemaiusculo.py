...
def tradução(palavra,tGregoMaiNome,tLatinoGregoMinusculo,tGregoMinMai):
    ...
    for letra in palavra:
        cont=0
        for (latino, gregoMinusculo) in tLatinoGregoMinusculo:
            if letra==latino:
                cont+=1
                for (gregomin, gregomai) in tGregoMinMai:
                    if gregomin == gregoMinusculo:
                        for (gregoMaiusculo,nome) in tGregoMaiNome:
                            if gregoMaiusculo==gregomai: 
                                dictfinal.update({
                                    letra:{'maiusculo': gregomai, 'minusculo': gregomin, 'nome': nome}
                                })
        if(cont==0):
            dictfinal.update({
                                letra:"sem correspondência"
                            })
